...
/**
 * Firewall Raw Resource Create Test
 */
func TestFirewallRaw_Create(t *testing.T) {

	// Initialize Resource Name
	resourceName := "mikrotik_firewall_raw.testacc"

	// Define Firewall Raw Values
	chain := "srcnat"
	disabled := false
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: testAccCheckFirewallRawDestroy,

		// Initialize Test Steps
		Steps: []resource.TestStep{
			{
				// Configure Test Resource
				Config: testAccFirewallRaw(
					chain,
					disabled,
				),
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccFirewallRawExists(resourceName),
 ...
 */
func testAccFirewallRawExists(resourceName string) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		// Find Resource by ID
		record, err := c.FindFirewallRaw(rs.Primary.ID)
 ...
 */
func testAccCheckFirewallRawDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// If Resource is not Firewall Raw
		if rs.Type != "mikrotik_firewall_raw" {
		...
		// Find Resource
		remoteRecord, err := c.FindFirewallRaw(rs.Primary.ID)
...
func testAccFirewallRaw(
	...
	return fmt.Sprintf(`
		resource "mikrotik_firewall_raw" "testacc" {
			chain = %q
			disabled = %t
		}
	`, chain, disabled)
