...
/**
 * Firewall Rule Resource Create Test
 */
func TestFirewallRule_Create(t *testing.T) {

	// Initialize Resource Name
	resourceName := "mikrotik_firewall_rule.testacc"

	// Define Firewall Rule Values
	chain := "input"
	disabled := false
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccFirewallRuleExists(resourceName),
 ...
 */
func testAccFirewallRuleExists(resourceName string) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		// Find Resource by ID
		record, err := c.FindFirewallRule(rs.Primary.ID)
...
func testAccCheckFirewallRuleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		// If Resource is not Firewall Rule
		if rs.Type != "mikrotik_firewall_rule" {
		...
		// Find Resource
		remoteRecord, err := c.FindFirewallRule(rs.Primary.ID)
...
func testAccFirewallRule(
	...
	return fmt.Sprintf(`
		resource "mikrotik_firewall_rule" "testacc" {
			chain = %q
			disabled = %t
		}
	`, chain, disabled)
